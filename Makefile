...
test: ##=> Run testkinesis
	@AWS_XRAY_CONTEXT_MISSING=LOG_ERROR $(PIPENV) run python tests/test_kinesis.py ${EVENTS}
...
_check_events_definition:
    $(info [*] Checking whether service $(EVENTS) exists...)

# EVENTS="<name_of_events>" must be passed as ARG for target or else fail
ifndef EVENTS
    $(error [!] EVENTS env not defined...FAIL)
endif
...
define HELP_MESSAGE
    ...
    EVENTS
        Default: not_defined
        Info: Environment variable to declare which event should be created
